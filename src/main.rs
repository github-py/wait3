...
use ansi_term::Colour::{Red, Fixed};
use ansi_term::Style;
...
enum Commands {
    ...
    #[command(arg_required_else_help(true), about = "waits for subprocess")]
    Subp {
        command: String,
        args: Vec<String>
    }
...
pub fn main() {
    ...
    match cli.command {
        Commands::Subp {command, args } => {
            ...
            let msg = Style::new().fg(Fixed(33)).bold().paint(&format!("Running command {}", Style::new().fg(Fixed(208)).bold().paint(label.clone()))).to_string();
        ...
        Commands::Time { amount } => {
            ...
            match smh.captures(&amount) {
                Some(caps) => {
                    ...
                    // 0x31 0x39 0x38 0x38
                    let msg = Style::new().fg(Fixed(220)).bold().paint(&format!("Waiting for {}", amount)).to_string();
